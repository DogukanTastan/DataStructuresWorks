...
class Message extends Account{
	 ...
	 Message(int messageId,Account sender,Account receiver,String content){
			...
			for(int i=0;i<receiver.getBlockedUsers_size();i++) {
				
				if(receiver.getBlockedUsers()[i].getUsername().equals(sender.getUsername())) {
					flag=true;
					break;
				}	
			}
			...
			if(flag) {
				...
				for(int i=0;i<receiver.getFollowers_size();i++) {
					
					if(receiver.getFollowers()[i].equals(sender.getUsername())) {
						 ...
						 super.outgoing_messages_size++;
						 receiver.addIncoming_messages(this);
						 super.incoming_messages_size++;
