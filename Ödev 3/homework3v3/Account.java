...
class Account {
	...
	protected LDLinkedList <String> history = new LDLinkedList<>();
	...
	public void login(LDLinkedList <Account> allAccounts) {
		boolean flag=false;
			...
			for(int i=0;i<obj.posts.size();i++) {
				...
				if(obj.posts.getBool(i) == false) {
					...
					for(int k=0;k< obj.posts.get(i).likes.size();k++) {
						if(obj.posts.get(i).likes.getBool(k) == false) {
					...
					for(int k=0;k< obj.posts.get(i).comments.size();k++) {
						if(obj.posts.get(i).comments.getBool(k) == false) {
	...
	public void viewProfile(Account obj){
		...
		boolean flag=false;
		int counter=0;
		System.out.println("View Profile: "+ obj.getUsername());
		for(int i=0;i<obj.blockedUsers.size();i++) {
			
			if(obj.blockedUsers.get(i).getUsername().equals(username) && obj.blockedUsers.getBool(i)== false) {
				flag=true;
				break;
			}	
		}
		...
		if(flag) {
			...
			for (int i=0; i<obj.following.size();i++) {
				if(obj.following.getBool(i) == true) 
					counter++;
			}
		
			System.out.println(obj.getUsername()+ " is following "+ (obj.following.size()-counter) +" account(s) and has "+
			obj.followers.size() + " follower(s).");
			
			System.out.print(obj.getUsername()+ " is following: ");
			
			for (int i=0; i<obj.following.size();i++) {
				if(obj.following.getBool(i) == false) 
	...
	public LDLinkedList<Account> getBlockedUsers() {  
		return blockedUsers;
	}
	public void setBlockedUsers(LDLinkedList<Account> blockedUsers) {
	...
	public  LDLinkedList<Message> getOutgoing_messages() {
		return outgoing_messages;
	}
	public void setOutgoing_messages( LDLinkedList<Message> outgoing_messages) {
		this.outgoing_messages = outgoing_messages;
	}
	public LDLinkedList<Message> getIncoming_messages() {
		return incoming_messages;
	}
	public void setIncoming_messages(LDLinkedList<Message> incoming_messages) {
